...
carrosReparados* novoCarroArvore(int idET, carro* carroReparado) { // recebe a et e o carro que foi arranjado
...
void inserirNaArvore(carrosReparados*& raiz, int idET, carro* carroReparado) {
    ...
    if (raiz == nullptr) {
        raiz = novoCarroArvore(idET, carroReparado);
        return;
    }
    if (idET < raiz->idET) {
        inserirNaArvore(raiz->esquerda, idET, carroReparado);
    }
    else if (idET > raiz->idET) {
...
void menu(marcas*& marca, modelos*& modelo, carro*& carros, estacoes*& estacao, carrosReparados*& raiz) {
    ...
    while (ficheiros)
    {
        ...
        case 'n':
            ...
            primeirosCarros(carros, marca, modelo, numCarrosTotal);

            estacaoTrabalho(estacao, numET, numEstacoes, marca);
    ...
    while (!sair) {
        ...
        case 's':
            ...
            criarCarro(carros, numCarrosTotal, estacao, marca, modelo);
        ...
        case 'g':
            ...
            gestao(estacao, numEstacoes, marca, numCarros, carros, numCarrosTotal, raiz);
